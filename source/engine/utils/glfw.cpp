...
namespace engine{
	...
	DelegateHandle engine::GLFWWindows::registerInitBody(std::function<bool(const GLFWWindows*)> function)
	{
		...
		{
			bResult &= function(windows); // bResult = bResult & function(windows);
		});
	}
	...
	bool GLFWWindowData::isMouseButtonPressed(MouseCode button) const
	{
		return false;
	}
	...
	void GLFWWindowData::callbackOnSetFoucus(bool bFoucus)
	{
	}
}
